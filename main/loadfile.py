...
def readbody():
	...
	train_out = np.float32(bodyfatTargets[0:200])
	train_in = np.float32(bodyfatInputs[0:200])
	test_out = np.float32(bodyfatTargets[200:253])
	test_in = np.float32(bodyfatInputs[200:253])
...
def readbuilding():
	...
	train_out = np.float32(buildingTargets[0:3400])
	train_in = np.float32(buildingInputs[0:3400])
	test_out = np.float32(buildingTargets[3400:4209])
	test_in = np.float32(buildingTargets[3400:4209])
...
def readartmfcc():
	...
	train_out = np.float32(art_V[0:12000])
	train_in = np.float32(mfcc_V[0:12000])
	test_out = np.float32(art_V[12000:16001])
	test_in = np.float32(mfcc_V[12000:16001])
...
def readmnist(ntrain=60000,ntest=10000,onehot=True):
	...
	train_x = np.float32(trX[:ntrain])
	trY = trY[:ntrain]

	test_x = np.float32(teX[:ntest])
	teY = teY[:ntest]

	if onehot:
		train_y = np.float32(one_hot(trY, 10))
		test_y = np.float32(one_hot(teY, 10))
	else:
		train_y = np.float32(np.asarray(trY))
		test_y = np.float32(np.asarray(teY))
...
def readcancer():
	...
	train_in = np.float32(inputs[:,0:550].transpose())
	train_out = np.float32(outputs[:,0:550].transpose())
	test_in = np.float32(inputs[:,550:701].transpose())
	test_out = np.float32(outputs[:,550:701].transpose())
